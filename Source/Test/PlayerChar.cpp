...
void APlayerChar::BeginPlay()
{
	...
	if (objWidget)
	{	
		objWidget->UpdatebuildObj(0.0f);//sets initial value to 0
...
void APlayerChar::Findobject()
{
			...
			if (Stamina > 5.0f) //checks to see if stamina is greater than 5
				...
				if (HitResource)
				{
					...
					if (HitResource->totalResource > ResourceValue) //if theres resource left we set the value
					{
						...
						matsCollected = matsCollected + ResourceValue; //adds to collected mats

						objWidget->UpdatematOBJ(matsCollected); //updates objective widget
						...
						SetStamina(-5.0f); //subtracts 5 from stamina
					}
					else
					{
						HitResource->Destroy();
	...
	else
	{
		...
		objectsBuilt = objectsBuilt + 1.0f; //adds to built objects

		objWidget->UpdatebuildObj(objectsBuilt); //updates objective widget
	}
